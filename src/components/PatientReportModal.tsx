...
const PatientReportModal: React.FC<Props> = ({
  patient, monthDate, company, shifts, nurses, shiftTypeDefs, onClose,
}) => {
  ...
  const safeFilename = `turnos-paciente-${patient.full_name.replace(/\s+/g, '-')}-${format(monthDate, 'yyyy-MM')}.pdf`;

  const buildPDF = () =>
    generatePatientPDF({ patient, shifts, nurses, shiftTypeDefs, monthDate, company });
    ...
    <div className="nrm-overlay" {...overlayClose}>
      <div className="nrm-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="nrm-header">
          <div>
            <h3 className="nrm-title">Reporte de Turnos — Paciente</h3>
            <p className="nrm-subtitle">{displayName} · {monthLabel}</p>
          </div>
