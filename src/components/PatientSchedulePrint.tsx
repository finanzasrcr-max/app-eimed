...
const PatientSchedulePrint: React.FC<Props> = ({
  patient,
  shifts,
  nurses,
  shiftTypeDefs,
  ...
  const patientShifts = shifts.filter(
    s => s.patient_id === patient.id &&
         isSameMonth(parseISO(s.start_at), monthDate) &&
         s.status !== 'cancelled'
  );

  const shiftsForDay = (day: Date) =>
    patientShifts.filter(s => isSameDay(parseISO(s.start_at), day));

  const usedTypes = shiftTypeDefs.filter(
    d => d.is_active && patientShifts.some(s => s.shift_type_id === d.id)
  );

  const displayName = patient.alias || patient.full_name;
      ...
      <div className="nsp-page">
        ...
        <div className="nsp-header">
          ...
          <div className="nsp-header-right">
            <span className="nsp-doc-title">CALENDARIO DE TURNOS — PACIENTE</span>
            <span className="nsp-nurse-name">{displayName}</span>
              ...
              return (
                ...
                >
                  ...
                  <div className="nsp-cell-shifts">
                    {dayShifts.map(shift => {
                      const nurse   = getNurse(shift.nurse_id);
                      const def     = getDef(shift.shift_type_id);
                      const name    =
                        nurse?.full_name.split(' ').slice(0, 2).join(' ') || '—';
...
export default PatientSchedulePrint;
